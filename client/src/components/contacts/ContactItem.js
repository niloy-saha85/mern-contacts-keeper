...
const ContactItem = () => {
  ...
  const { updateContact, selectedContact, contactError, contactErrorMsg } = contactContext;
  const [success, setSuccess] = useState(false);
  const [inputs, setInputs] = useState(selectedContact);
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    console.log(inputs);
    const res = await updateContact(inputs);
    setSuccess(res);
    if (res) {
      setTimeout(() => {
        setSuccess(false)
          ...
          <div className="card card-secondary">
            ...
            <div className="card-body">
              <div className="form-group">
                ...
                <input type="text" name="name" value={inputs.name} className="form-control" onChange={onChange} required />
              </div>
              <div className="form-group">
                <label htmlFor="Email">Email</label>
                <input type="email" name="email" value={inputs.email} className="form-control" onChange={onChange} required />
              </div>
              <div className="form-group">
                <label htmlFor="phone">Phone</label>
                <input type="number" name="phone" value={inputs.phone} className="form-control" onChange={onChange} required />
              </div>
              <div className="form-group">
